 ...
 * AUTO-MATCH: cherche la resa PassPass sortante (end === date) ou active dans Supabase.
 */
...
if (!BOT_TOKEN || !SUPABASE_URL || !SUPABASE_KEY) {
  ...
  // AUTO-MATCH: cherche et enrichit avec les infos PassPass si disponibles
  async function sauver(logement, description, typeSinistre, photos, firstMsg) {
    ...
    // Matcher le logement avec un property_id (bidirectionnel: "Février 2" match "Fevrier 2 - Lyon")
    let property_id = '';
    if (logement && state.properties && state.properties.length) {
      ...
      console.log('Property match:', logement, '->', property_id ? 'trouvé' : 'aucun');
    }
  ...
  async function finaliserBuffer(userId) {
    ...
    // 2. Legende courte (<=50 chars) = nom du logement (ex: "Février 2", "BERLIOZ 69100")
    // Note: ne pas filtrer les noms de mois - "Février 2" est un nom d'appartement valide
    let logement = null;
